...
class SeaGlider:
    ...
    def __repr__(self):
        ...
        string += "\n"
        plot_vars = list(self.variables.keys())
...
class SG_var(object):
    ...
    def __getitem__(self, key):
        ...
        all_data = []
        for i, fname in enumerate(self.__files__[key]):
            nco = open_dataset(fname)
            var = nco[self.name].to_masked_array()
            data = DataFrame(var, columns=[self.name])
            data['dive_index'] = i
            data['depth'] = nco['depth'].values
            data['time'] = nco['time'].values
            all_data += data,
